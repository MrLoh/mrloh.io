...
export default async function AboutPage({ searchParams }: AboutPageProps) {
  ...
  return (
    <div className="relative mt-6 flex w-full max-w-none flex-col">
      <h1
        className={twJoin(
          'prose-max prose-px my-8 flex justify-between',
          'text-3xl font-bold text-zinc-700 italic dark:text-zinc-300',
        )}
      >
        <div>Tobias Lohse</div>
      ...
      <Markdown source={intro} className="prose-max prose-px" />
      <Experiences
        experiences={experiences}
        className="prose-max prose-px mt-8"
        action={
          <Button href="/resume.pdf" className="grow-0 basis-auto gap-1.5 px-3 py-1 text-xs">
            download resume
