...
Forms should know how to handle invalid input results. If the submission returns field errors, our form component automatically maps them to fields and renders inline messages. Anything else belongs to a page or dialog-level error helper.

That split keeps form code boring. Forms render field errors. Everything else belongs to a different UI boundary.

The types make this split hard to violate. The form component’s post-submit branch is three lines: success, invalid input (mapped to fields), or anything else (handed by a dialog helper). The page-level error type is the rejection union with invalid input excluded, so a page renderer can’t consume a field error and a form can’t render one as a dialog.
